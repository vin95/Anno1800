...
class validateActionDrawResidentCardTest {
    ...
    private  Action action = new Action.DrawResidentCard();
    private Player player = game.getCurrentPlayer();

    @Test
    @DisplayName("At the start of the game, a player can always draw a resident card")
    void assertTrue(ActionValidator.canDrawResidentCard(action, player, game), "Player should be able to draw a resident card at the start of the game.");
}